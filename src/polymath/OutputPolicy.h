...
public:
	...
	void OutputStartVertex(OutputEdge &edge1, OutputEdge &edge2, VertexType vertex, OutputEdge *prev_edge, OutputEdge *next_edge, bool is_split) {
		POLYMATH_UNUSED(prev_edge);
		POLYMATH_UNUSED(next_edge);
	...
	void OutputMiddleVertex(OutputEdge &edge, VertexType vertex) {
	...
	void OutputStopVertex(OutputEdge &edge1, OutputEdge &edge2, VertexType vertex) {
...
public:
	...
	void OutputStartVertex(OutputEdge &edge1, OutputEdge &edge2, VertexType vertex, OutputEdge *prev_edge, OutputEdge *next_edge, bool is_split) {
		...
		if(is_split) {
			...
			OutputPolygon *output_polygon1 = prev_edge->m_output_polygon;
			OutputPolygon *output_polygon2 = output_polygon1->m_opponent; //next_edge->m_output_polygon;
			if(output_polygon2 == nullptr) {
				...
				if(output_polygon1->m_last_forward) {
					...
					next_edge->m_output_polygon = output_polygon;
					...
					edge2.m_output_forward = false;
					prev_edge->m_output_polygon = output_polygon;
