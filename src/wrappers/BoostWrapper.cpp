...
void PolyToBoost(const Polygon &input, boost_polygons &output) {
	output.clear();
	...
	for(size_t i = 0; i < input.GetLoopCount(); ++i) {
		...
		if(input.GetLoopWindingWeight(i) > 0) {
			...
			for(size_t j = 0; j < n; ++j) {
				ring[j] = boost_point(vertices[j].x(), vertices[j].y());
			}
		} else {
			boost_ring ring;
			ring.resize(n);
...
void PolyFromBoost(const boost_polygons &input, Polygon &output) {
	output.Clear();
