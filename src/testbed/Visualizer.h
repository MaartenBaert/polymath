		...
		if(m_visualization.m_has_current_vertex) {
			...
			for(typename Visualization::SweepEdge &edge : m_visualization.m_sweep_edges) {
				...
				if(edge.m_has_helper) {
					...
					painter.drawLine(QPointF(double(edge.m_helper_vertex.x) * mult, double(edge.m_helper_vertex.y) * mult), QPointF(double(m_visualization.m_current_vertex.x) * mult, cy * mult));
				}
			}
